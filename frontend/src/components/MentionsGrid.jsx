// src/components/MentionsGrid.jsx
import React from "react";
import { Globe } from "lucide-react";

const MentionsGrid = ({ mentions }) => {
  console.log("MentionsGrid received:", mentions); // Debug log
  ...
  const getSentimentClass = (sentiment) => {
    ...
    const sentimentLower = label.toLowerCase();
  ...
  return (
    <div className="bg-white rounded-xl p-6 border border-gray-200">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-sm text-gray-500">Showing {mentions.length} mentions</span>
      </div>
      <div className="space-y-3 max-h-[600px] overflow-y-auto">
        {mentions.map((mention, idx) => {
          const sentimentLabel = getSentimentLabel(mention.sentiment);
            ...
            >
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {mention.content && (
                    <p className="text-xs text-gray-600 line-clamp-2 mt-1">{mention.content}</p>
                  )}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs text-gray-500">
                  ...
                  {mention.topics && mention.topics.length > 0 && (
                    <span>• {mention.topics[0]}</span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-gray-500">{formatDate(mention.timestamp)}</span>
                  <span className={`px-3 py-1 rounded-md text-xs font-medium ${getSentimentClass(mention.sentiment)}`}>
